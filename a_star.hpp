...
namespace libMultiRobotPlanning {
    ...
    */
    ...
    public:
        ...
        {
			std::string logFileName = "p/log" + std::to_string(logId) + ".txt";
            ...
            PlanResult<State, Action, Cost>& solution, Cost initialCost = 0)
        {
            ...
            /*std::stringstream ss;
            ss << "[" << startState.state.x << "-" << startState.state.y << "]" << "," << "f:" << 0 << "," << "g:" << 0;
                ...
                for (const Neighbor<State, Action, Cost>& neighbor : neighbors)
                {
                    if (closedSet.find(neighbor.state) == closedSet.end())
                    {
                        ...
                        {  // Discover a new node
                            ...
                            /*std::stringstream ss;
                            ss<< "[" << neighbor.state.state.x << "-" << neighbor.state.state.y <<"]"<<"," << "f:" << fScore << "," << "g:" << tentative_gScore;
                            std::string log = ss.str();
                            AppendLog(log);*/
                        }
                        else
                        {
                            ...
                            m_env.onDiscover(neighbor.state, (*handle).fScore,
                                (*handle).gScore);
                           /* std::stringstream ss;
                            ss<< "[" << neighbor.state.state.x << "-" << neighbor.state.state.y <<"]"<< "," << "f:" << (*handle).fScore << "," << "g:" << (*handle).gScore;
                            std::string log =ss.str();
...
#else
            typename boost::heap::d_ary_heap<Node, boost::heap::arity<2>,boost::heap::mutable_<true>>::handle_type
                handle;
#endif
        };
    ...
    int AStar<State, Action, Cost, Environment, StateHasher>::logId = 0;
